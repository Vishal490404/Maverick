...
@router.post("/topics/{topic_id}/questions", response_model=QuestionResponse, status_code=status.HTTP_201_CREATED)
async def create_question(
    ...
    # Check if question type exists
    question_type = db.question_types.find_one({"id": question.question_type_id})
    if not question_type:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Question type not found"
    ...
    # Get chapter and subject info for the question
    chapter = db.chapters.find_one({"id": topic["chapter_id"]})
    if not chapter:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Chapter not found for this topic"
        )
    
    subject = db.subjects.find_one({"id": chapter["subject_id"]})
    if not subject:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Subject not found for this chapter"
        )
    ...
    if question_type["has_options"]:
        if not question.options or len(question.options) == 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="This question type requires options"
            )
        
        # Check if we have the right number of options
        if question_type["option_count"] and len(question.options) != question_type["option_count"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"This question type requires exactly {question_type['option_count']} options"
            )
    ...
    question_dict["topic_id"] = topic_id
    question_dict["chapter_id"] = topic["chapter_id"]
    question_dict["subject_id"] = chapter["subject_id"]
    question_dict["standard_id"] = subject["standard_id"]
    ...
    question_dict["updated_by"] = current_user.username
    
    # Add question type info for convenience
    question_dict["question_type_name"] = question_type["name"]
...
async def update_question(
    ...
    existing_question = db.questions.find_one({"id": question_id})
    ...
    # If question type is being changed, validate it
    if question_update.question_type_id and question_update.question_type_id != existing_question["question_type_id"]:
        # Check if new question type exists
        question_type = db.question_types.find_one({"id": question_update.question_type_id})
        if not question_type:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Question type not found"
        ...
        # Update question type info
        update_data = {k: v for k, v in question_update.model_dump().items() if v is not None}
        update_data["question_type_name"] = question_type["name"]
        
        # Check options compatibility with new question type
        options = question_update.options if question_update.options is not None else existing_question.get("options", [])
        
        if question_type["has_options"] and (not options or len(options) == 0):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="The selected question type requires options"
            )
        
        if question_type["option_count"] and options and len(options) != question_type["option_count"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"The selected question type requires exactly {question_type['option_count']} options"
            )
    else:
        update_data = {k: v for k, v in question_update.model_dump().items() if v is not None}
    ...
    updated_question = db.questions.find_one({"id": question_id})
...
async def search_questions(
    ...
    query_filter = {}
    if standard_id:
        query_filter["standard_id"] = standard_id
    if subject_id:
        query_filter["subject_id"] = subject_id
    if chapter_id:
        query_filter["chapter_id"] = chapter_id
    if topic_id:
        query_filter["topic_id"] = topic_id
    if difficulty_level:
        query_filter["difficulty_level"] = difficulty_level
    if question_type:
        query_filter["question_type_id"] = question_type
    if marks:
        query_filter["marks"] = marks
